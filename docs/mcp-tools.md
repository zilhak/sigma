...
**검사 규칙**: `outside_section`(섹션 밖 배치노드) · `section_overlap` · `card_overlap`(섹션 안 프레임 겹침) · `frame_padding`(섹션 안 프레임 ≥20px 여백) · `instance_orphan`(섹션/페이지 직속으로 뜬 인스턴스 — 마스터 내부 중첩은 정상, anno/wire 예외) · `component_needs_frame`(섹션 직속 COMPONENT/GROUP 금지 — 프레임 안에) · `child_overflow`(트리상 자식이면 좌표상으로도 부모 안 — 모든 컨테이너가 자식 로컬좌표라 부모 로컬박스 0,0~W,H 기준, 섹션도 동일, 배치형만·리프 제외).
