...
const sigmaAPI = {
  ...
  // alt 속성으로 이미지 찾기
  findByAlt: function (altText: string): ElementInfo | null {
  ...
  // 텍스트 내용으로 요소 찾기
  findByText: function (text: string, tagName?: string): ElementInfo | null {
  ...
  // form 요소 찾기
  findForm: function (action?: string): ElementInfo | null {
  ...
  findContainer: function (options: ContainerOptions): ElementInfo | null {
  ...
  // 셀렉터로 요소 정보 조회
  getElementInfo: function (selector: string): ElementInfo | null {
    const el = document.querySelector(selector);
    return el ? getElementInfo(el) : null;
  },

  // 페이지 구조 요약
  getPageStructure: function (): PageStructure {
    ...
    const buttons = Array.from(document.querySelectorAll('button, input[type="submit"], [role="button"]'))
                    .slice(0, 10).map(getElementInfo);
    const links = document.querySelectorAll('a').length;

    // 메인 콘텐츠 영역 추정 (main, article, 또는 가장 큰 콘텐츠 영역)
