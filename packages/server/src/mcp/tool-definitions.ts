...
\`args\` 안의 키도 메서드별로 검사합니다 — 모르는 키를 주면 \`acceptedArgs\`와 함께 거부됩니다(조용한 no-op 방지).
  ...
  // === Component Spec System (스펙 기반 컴포넌트) ===
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        overwrite: { type: 'boolean', description: '(선택) 같은 namespace+alias가 있으면 in-place 갱신(인스턴스 전파). 기본 false. 갱신 시 응답의 `impact`(크기 변화 + 인스턴스가 몇 개·어느 페이지에 있는지)를 반드시 확인하세요 — 크기가 바뀌면 그 페이지들의 주석 마커가 어긋납니다' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        force: { type: 'boolean', description: '(선택) deleteNode 인데 인스턴스가 남아 있어도 강행. 기본 false — 남은 인스턴스가 전부 깨지므로 개수와 페이지를 응답에 담아 거부합니다' },
      },
      required: ['token', 'alias'],
    },
  },
...
기하 규칙 위반에는 \`metrics\`(초과량·컨테이너 크기·겹침 폭높이·요구 여백)가 실립니다 — 초과량을 알려고 \`sigma_get_tree\` 를 다시 부르지 마세요. **\`message\` 를 정규식으로 파싱하지 말고 \`metrics\` 에서 읽으세요.**
