...
## ⛔ 위 「동시 전수 조회」 진단은 [023](023-read-concurrency-did-not-reproduce-the-death.md) 실측이 뒤집었다

같은 날 실측했더니 **읽기 동시성만으로는 재현되지 않았다.** 이 문서의 사망 조건을 모든
축에서 넘겨도(동시 14 · 동시총량 48.7MB · 누적 708MB · 수명 6,656초) 죽지 않았고,
스케일링은 초선형이 아니라 **선형**이었다. `pending: 6` 은 사망과 **상관**이 있었을 뿐
원인이라는 근거가 아니었다.

⇒ 아래 「남은 것」의 **큐 계획을 023 을 보지 않고 그대로 구현하지 말 것.** 남은 유력 후보는
**쓰기 경로**다(이 문서의 명령 분포에서도 `MODIFY_NODE×571` 이 최다였고, 치명 스택도 쓰기였다).

## 남은 것

- ~~**동시 예산 실측**~~ → [023](023-read-concurrency-did-not-reproduce-the-death.md) 에서 했다. **음성 결과.**
  재현 도구는 `.claude-workspace/plugin-budget/concurrency-probe.ts`.
- **읽기+쓰기 혼합 재현** ← 다음 단계. 023 의 프로브는 읽기만 던져 `MODIFY_NODE` 축을 안 건드렸다.
  사용자 파일을 변형하므로 실험 전용 페이지에서만 돌린다.
- **전송 구간에 예산이 없다** (023 이 015 의 미해결 질문을 닫으며 드러난 것). 순회는 55초에
  멈추는데 직렬화+청킹+전송이 80초 더 걸려, 호출자는 «부분 결과» 가 아니라 **아무것도** 못 받는다.
- ~~**무거운 명령 동시 상한(큐)**~~ — 근거를 잃었다(위 참조). 되살린다면 그때는 **읽기가 아니라
  쓰기** 쪽에 근거가 있는지부터 볼 것. 만들 때의 주의사항만 남겨 둔다: 전부 직렬화는 안 되고
  (14초짜리 전수 스캔이 20ms 짜리 rename 을 막는다), **타임아웃 시작점을 «전송 시점» 으로 옮겨야
  하며**(지금은 큐에 넣는 순간부터 세므로 대기 중에 타임아웃이 난다), 대기 시간을 로그에 남겨야
  «느린 게 큐인지 플러그인인지» 를 가른다.
