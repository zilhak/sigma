...
| Lint | `sigma_lint` | config 기반 빌트인 24종(기본 ON 15 + opt-in 9)+커스텀 규칙 검사, 안전 자동수정(`apply:true`), 전체 파일 검사(`scope:"file"` → md 리포트) |
...
| [Lint](docs/lint/) | 빌트인 24종(룰 1개 = 파일 1개) + 커스텀 규칙(JSON/predicate) 스키마, config 예제, 설계 근거 |
