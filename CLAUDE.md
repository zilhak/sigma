...
**특정 구현에 사유가 있으면(겪은 상황·사고·실측) `docs/history/` 에 경험 기록용 md 파일을
만들고 그것을 참조하며 작업한다.**

코드 주석은 "지금 이게 무엇인가"를 적기에 좋지만, **"무엇을 겪어서 이렇게 됐는가"는 담을 자리가 없다.**
그 맥락이 사라지면 다음 사람이 같은 사고를 다시 내거나, 사고를 막으려고 넣은 방어를 "쓸데없어 보인다"며 되돌린다.

- **파일명**: `docs/history/NNN-<주제-kebab>.md` (번호는 이어서). 한 사건 = 한 파일.
- **언제 쓰나**: 아래 중 하나라도 해당하면 쓴다.
  - 사고·오작동을 겪고 그 때문에 코드가 지금 모양이 됐다
  - 실측 수치가 판단 근거다 (응답 크기, 소요 시간, 위반 건수 등)
  - "안 그래 보이는데 이렇게 해야 하는" 반직관적 결정을 했다
  - 어떤 접근을 시도했다가 물렸다 (되돌린 이유가 다음 사람에게 필요하다)
- **무엇을 쓰나**: 증상 → 원인 → 무엇을 했나 → **되돌리면 안 되는 이유**. 추측과 실측을 구분해 적는다.
- **코드에서 가리킨다**: 방어 코드·이상해 보이는 분기 옆에 `// 배경: docs/history/003-….md` 한 줄을 남긴다.
  기록만 있고 코드가 안 가리키면 아무도 찾지 못한다.
- **TODO 를 구현할 때**: TODO 본문에 겪은 사항(사고 이력·실측)이 들어 있으면 **그걸 옮겨 적는다.**
  TODO 는 완료 후 지워지므로, 남기지 않으면 근거가 통째로 사라진다.
- **쓰지 않는 것**: 코드를 읽으면 아는 것, 커밋 메시지로 충분한 단순 변경, 진행 중 메모
  (그건 `.claude-workspace/`). 기록을 위한 기록은 만들지 않는다.
