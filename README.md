...
| Lint | `sigma_lint` | config 기반 빌트인 25종(기본 ON 16 + opt-in 9)+커스텀 규칙 검사, 안전 자동수정(`apply:true`), 전체 파일 검사(`scope:"file"` → md 리포트) |
| 조회 | `sigma_find_node` / `sigma_get_tree` | 노드 검색(경로 또는 `where` 속성 조건) / 계층 탐색 |
| | `sigma_get_node_info` / `sigma_get_nodes_info` | 노드 상세 정보 (단일/배치) |
| | `sigma_get_document_info` | 문서 정보 (파일명, 페이지 목록) |
...
| [Lint](docs/lint/) | 빌트인 25종(룰 1개 = 파일 1개) + 커스텀 규칙(JSON/predicate) 스키마, config 예제, 설계 근거 |
