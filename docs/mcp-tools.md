...
**검사 규칙**: `outside_section`(섹션 밖 배치노드) · `section_overlap` · `card_overlap`(섹션 안 프레임 겹침) · `frame_padding`(섹션 안 프레임 ≥20px 여백) · `instance_orphan`(섹션/페이지 직속으로 뜬 인스턴스 — 마스터 내부 중첩은 정상, anno/wire 예외) · `child_overflow`(트리상 자식이면 좌표상으로도 부모 안 — 섹션은 절대좌표·프레임은 로컬좌표 기준, 배치형만·리프 제외).
