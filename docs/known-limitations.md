...
Sigma는 원본의 `font-family`를 따라간다 — 변환 전에 트리가 쓰는 패밀리/weight를 Figma에
로드하고(`converter/font-loader.ts`), 텍스트 노드에 그 폰트를 적용한다(`node-creator.ts`).
CSS가 폰트를 지정하지 않았으면 **파일 기본 폰트**를 쓴다
(문서 설정 `fonts.default` — `sigma_set_page_data(pageId:"document", key:"fonts")`, 미설정 시 Inter).

남는 한계는 **그 폰트가 실행 환경에 설치돼 있지 않은 경우**다. 이때는 파일 기본 폰트 →
Inter 순으로 폴백하는데, 폴백 폰트의 글리프 너비가 다르면 **텍스트 노드의 크기가 달라지고**,
이것이 부모 레이아웃에 영향을 준다.
...
| 폰트 미설치로 폴백 | 텍스트 크기 → 레이아웃 변경 | 텍스트만 다름 (레이아웃 무관) |
